{
  ...
  let
    ...
    # Load metadata from SOPS encrypted JSON file
    # Falls back to environment variables if file doesn't exist
    metadataPath = ./metadata.json;

    loadMetadata =
      if builtins.pathExists metadataPath then
        let
          # Check if SOPS_AGE_KEY_FILE is set
          sopsKeyFile = ./sopskey.txt;
          sopsKeyExists = sopsKeyFile != "" && builtins.pathExists sopsKeyFile;

          # Decrypt the metadata.json file using sops
          decryptedJson = if sopsKeyExists then
            builtins.fromJSON (builtins.readFile (
              pkgs.runCommand "decrypt-metadata" {
                buildInputs = [ pkgs.sops ];
              } ''
                export SOPS_AGE_KEY_FILE="${sopsKeyFile}"
                ${pkgs.sops}/bin/sops -d ${metadataPath} > $out
              ''
            ))
          else
            # If key file doesn't exist, return empty set and warn
            builtins.trace "WARNING: SOPS_AGE_KEY_FILE not set or file doesn't exist. Set it to decrypt metadata.json" {};
        in
          decryptedJson
      else
        # metadata.json doesn't exist, return empty set
        builtins.trace "INFO: metadata.json not found at ${toString metadataPath}. Using environment variables or template placeholders." {};

    launchReplacements =
      let
        # Priority: 1. SOPS metadata.json, 2. Environment variables, 3. Template placeholders
        getValue = metadataKey: envName: default:
          let
            # Check if metadata has the key
            metadataValue = if loadMetadata ? ${metadataKey} then loadMetadata.${metadataKey} else "";
            # Check environment variable
            envValue = builtins.getEnv envName;
          in
            if metadataValue != "" then metadataValue
            else if envValue != "" then envValue
            else default;
      in {
        robotId = getValue "ROBOT_ID" "ROBOT_ID" "{{ROBOT_ID}}";
        signalingUrl = getValue "SIGNALING_URL" "SIGNALING_URL" "{{SIGNALING_URL}}";
        password = getValue "PASSWORD" "PASSWORD" "{{PASSWORD}}";
        githubUser = getValue "GITHUB_USER" "GITHUB_USER" "{{GITHUB_USER}}";
        turnServerUrl = getValue "TURN_SERVER_URL" "TURN_SERVER_URL" "{{TURN_SERVER_URL}}";
        turnServerUsername = getValue "TURN_SERVER_USERNAME" "TURN_SERVER_USERNAME" "{{TURN_SERVER_USERNAME}}";
        turnServerPassword = getValue "TURN_SERVER_PASSWORD" "TURN_SERVER_PASSWORD" "{{TURN_SERVER_PASSWORD}}";
      };

    launchRewriteHelper = ''
      export ROBOT_ID="${launchReplacements.robotId}"
      export SIGNALING_URL="${launchReplacements.signalingUrl}"
      export PASSWORD="${launchReplacements.password}"
      export GITHUB_USER="${launchReplacements.githubUser}"
      export TURN_SERVER_URL="${launchReplacements.turnServerUrl}"
      export TURN_SERVER_USERNAME="${launchReplacements.turnServerUsername}"
      export TURN_SERVER_PASSWORD="${launchReplacements.turnServerPassword}"
      ...
      applyLaunchRewrites() {
        ...
        if [ ! -d "$target" ]; then
          echo "[launch-rewrite][$label] target not found: $target" >&2
          return
        fi

        ${pkgs.python3}/bin/python3 - "$target" "$label" <<'PY'
...
repls = {
    ...
    "{{TURN_SERVER_URL}}": os.environ["TURN_SERVER_URL"],
    "{{TURN_SERVER_USERNAME}}": os.environ["TURN_SERVER_USERNAME"],
    "{{TURN_SERVER_PASSWORD}}": os.environ["TURN_SERVER_PASSWORD"],
}

print(f"[launch-rewrite][{label}] replacements:", file=sys.stderr)
for k, v in repls.items():
    unresolved = v.strip() == "" or (v.startswith("{{") and v.endswith("}}"))
    note = " (unresolved?)" if unresolved else ""
    print(f"  {k} -> {v}{note}", file=sys.stderr)

for path in root.rglob("*.launch.py"):
    try:
        original = path.read_text()
    except Exception as exc:  # pragma: no cover
        print(f"[launch-rewrite][{label}] failed to read {path}: {exc}", file=sys.stderr)
        continue
    ...
    if replaced_keys:
        ...
        remaining = [k for k in repls if k in rewritten]
        print(f"[launch-rewrite][{label}] updated {path} (replaced: {sorted(set(replaced_keys))}; remaining: {remaining})", file=sys.stderr)
    else:
        print(f"[launch-rewrite][{label}] no placeholders in {path}", file=sys.stderr)
PY
      }
    '';
        ...
        workspacePackages = lib.mapAttrs (pkgName: pkgInfo:
          ...
          pkgs.python3Packages.buildPythonPackage {
            ...
            postInstall = ''
              ...
              ${launchRewriteHelper}
              applyLaunchRewrites "$out/share/$pkg" "$pkg"
...
EOF
        ...
        workspaceWithLaunch = pkgs.runCommand "${name}-with-launch" {} ''
          ...
          ${launchRewriteHelper}
          applyLaunchRewrites "$out/share" "${name}-workspace"
        '';
  ...
  in
  {
    ...
    nixosConfigurations.rpi4 = nixpkgs.lib.nixosSystem {
      inherit system;
      specialArgs = {
        inherit pyEnv robotConsole robotApi rosWorkspace rosRuntimeEnv systemRosWorkspace systemRosRuntimeEnv launchReplacements;
      };
      modules = [
        ({ ... }: {
          nixpkgs.overlays =
